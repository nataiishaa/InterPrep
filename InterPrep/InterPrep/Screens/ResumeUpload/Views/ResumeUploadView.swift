...
public struct ResumeUploadView: View {
    ...
    private var supportedFormatsSection: some View {
        VStack(spacing: Layout.supportedFormatsSectionSpacing) {
            ...
            HStack(spacing: Layout.supportedFormatsRowSpacing) {
                ForEach(["PDF", "DOCX", "RTF", "TXT"], id: \.self) { format in
                    Text(format)
                        .font(.caption2)
                        .fontWeight(.medium)
                        .foregroundColor(.textPrimary)
            ...
            Text("Максимальный размер: 20 МБ")
                .font(.caption2)
                .foregroundColor(.textTertiary)
    ...
    private var bottomButtons: some View {
        VStack(spacing: Layout.bottomButtonsVStackSpacing) {
            if model.selectedFile != nil && model.uploadStatus != .success && model.uploadStatus != .awaitingQuestions {
                Button {
                    model.onUpload()
                } label: {
                    if model.uploadStatus == .uploading {
    ...
    func makeUIViewController(context: Context) -> UIDocumentPickerViewController {
        let picker = UIDocumentPickerViewController(
            forOpeningContentTypes: [
                ...
                .plainText,
                .rtf,
                UTType(filenameExtension: "docx") ?? UTType("org.openxmlformats.wordprocessingml.document") ?? .data
            ]
        )
