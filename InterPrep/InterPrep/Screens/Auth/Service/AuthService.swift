...
public final class AuthService: AuthServicing {
    ...
    @MainActor
    private func connectionError() -> AuthError {
        NetworkMonitor.shared.isConnected ? .vpnLikely : .networkUnavailable
    }
    ...
    public func login(email: String, password: String) async throws {
        ...
        case .failure(let error):
            if error.isConnectionError {
                throw await connectionError()
            }
            if let api = error.asAPIError {
